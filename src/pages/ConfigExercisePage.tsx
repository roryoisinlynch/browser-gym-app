          ...
          <div className="config-exercise__label-row">
            ...
            <span ref={weightModeTooltipRef} style={{ position: "relative" }}>
              ...
              {weightModeTooltipOpen && (
                <div className="config-exercise__info-tooltip">
                  <strong>Bodyweight</strong> — for exercises where the only variable you change week to week is the number of reps, such as pull-ups. Also useful for high-rep exercises where e1RM calculations become unreliable. In this scenario you would create a named exercise with a fixed weight e.g. "2.5kg Lateral Raise", set it to bodyweight mode, and progress in reps only. When you're ready to progress to the next weight, create a "5kg Lateral Raise" exercise template to replace it.<br />
                  <strong>Increment</strong> — The most common choice. Use this for exercises for which you have weight choices which progress in even increments. For example, if you're using a barbell and have the option to add or remove weights in 2.5kg increments. Setting 2.5kg in this field means the app can prescribe any weight in the sequence 2.5kg, 5kg, 7.5kg, 10kg…<br />
                  <strong>Fixed list</strong> — a workaround for equipment with uneven increments, such as a cable machine that jumps from 1.25kg to 5kg to 8kg. You specify each available weight individually and select from that list each session. <em>e.g. if you enter 5kg, 8kg, 12kg, the app will only ever prescribe one of those three weights.</em>
                </div>
              )}
            </span>
          </div>
