...
export default function SessionPage() {
  ...
  // Upper bound for the per-card season-sets bar. The scale is local to this
  // session instance, so a bar answers "how does this exercise compare to the
  // others I'm doing today" rather than an absolute question. Read off
  // `sessionView` rather than the rendered `sortedMuscleGroups` so collapsing a
  // group doesn't rescale everything.
  const maxSeasonSets = useMemo(() => {
    let max = 0;
    for (const group of sessionView?.muscleGroups ?? []) {
      for (const exercise of group.exercises) {
        max = Math.max(max, exercise.workingSetsThisSeason);
      }
    }
    return max;
  }, [sessionView]);
                    ...
                    >
                      ...
                      {!isCollapsed && (
                        <ul className="exercise-list">
                          {exercises.map(
                            ({ sessionInstanceExerciseId, exerciseTemplate, exerciseInstance, sets, prescribedWeight, prescribedRepTarget, effectiveE1RM, isDormant, workingSetsThisSeason }) => {
                              ...
                              // Sets-this-season bar, scaled against the busiest exercise in the
                              // session so the bar reads as "how much of today's heaviest column
                              // is this one".
                              const seasonSetsPct =
                                maxSeasonSets > 0
                                  ? clampPercentage(
                                      (workingSetsThisSeason / maxSeasonSets) * 100
                                    )
                                  : 0;
