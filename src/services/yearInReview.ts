...
/**
 * "Now" for every year-in-review gate. In dev builds it honors a localStorage
 * override ("yearReviewNow", any Date-parsable string) so the window and year
 * logic can be exercised without changing the system clock; production ignores
 * it (and any stale key a testing device still carries). To show the feature
 * outside the real window in production, use the hidden `/year-in-review?preview`
 * deck instead. Dead-code-eliminated in production.
 */
