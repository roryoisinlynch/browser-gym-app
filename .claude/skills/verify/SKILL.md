...
- One-time flags live in the `meta` store (`browser-gym-app` DB), e.g. `year_in_review_prompt_seen_<year>`. Read/delete them via `page.evaluate` with plain indexedDB to test once-only behavior.
